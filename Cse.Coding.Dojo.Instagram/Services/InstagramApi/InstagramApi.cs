...
namespace Cse.Coding.Dojo.Instagram.Services.InstagramApi
{
    public class InstagramApi
    {
        ...
        public void SetBuild(UserSessionData userSessionData)
            => _instaApiSdk = InstaApiBuilder.CreateBuilder()
                .SetUser(userSessionData)
                .UseLogger(new DebugLogger(LogLevel.All))
                .SetRequestDelay(RequestDelay.FromSeconds(2, 2))
                .Build();

        public async Task<InstaLoginResult> InstagramLoginAsync()
            => await ExtractResultFromFunctionAsync(_instaApiSdk.LoginAsync());
